...
async function getAvailability(restaurantId, date, guests) {
  ...
  const { data: tables } = await supabase.from('tables').select('*').eq('restaurant_id', restaurantId).eq('active', true).gte('capacity', guests);
  if (!tables || tables.length === 0) return [];
  const slots = generateSlots(opening, closing, duration);
  const available = [];
  for (const slot of slots) {
    const { data: existing } = await supabase.from('reservations').select('table_id').eq('restaurant_id', restaurantId).eq('date', date).eq('time', slot).eq('status', 'confirmed');
...
async function createReservation(restaurantId, data) {
  ...
  const { data: tables } = await supabase.from('tables').select('*').eq('restaurant_id', restaurantId).eq('active', true).gte('capacity', data.guests);
  if (!tables || tables.length === 0) return { error: 'No hay mesas' };
  const { data: existing } = await supabase.from('reservations').select('table_id').eq('restaurant_id', restaurantId).eq('date', data.date).eq('time', data.time).eq('status', 'confirmed');
  const bookedTableIds = (existing || []).map(r => r.table_id);
  const freeTable = tables.find(t => !bookedTableIds.includes(t.id));
  ...
  const insertData = { restaurant_id: restaurantId, table_id: freeTable.id, customer_phone: data.customer_phone, customer_name: data.customer_name, date: data.date, time: data.time, end_time: endDateTime.toISOString(), guests: data.guests, status: 'confirmed' };
  console.log('Insertando:', insertData);
  const { data: res, error } = await supabase.from('reservations').insert([insertData]).select();
...
async function cancelByPhone(phone) {
  const { data } = await supabase.from('reservations').update({ status: 'cancelled' }).eq('customer_phone', phone).eq('status', 'confirmed').select().maybeSingle();
  return data;
}
